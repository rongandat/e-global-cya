<?php
	...
	include(_INCLUDES_DIR.'Smarty/libs/Smarty.class.php');
	
	include(_INCLUDES_DIR.'dbtables.php');	
	...
	define('_CURRENT_ADMIN_MODULE',isset($_GET['module']) ? $_GET['module'] : 'home');
	define('_CURRENT_ADMIN_PAGE',isset($_GET['page']) ? $_GET['page'] : 'index');
	define('_CURRENT_ADMIN_PAGE_LINK',get_admin_link('module='._CURRENT_ADMIN_MODULE.'&page='._CURRENT_ADMIN_PAGE));
		...
		// verify the ssl_session_id if the feature is enabled
		  if ( ($request_type == 'SSL') && (SESSION_CHECK_SSL_SESSION_ID == 'True') && (ENABLE_SSL == true) && ($session_started == true) ) {
			...
			if ($SESSION_SSL_ID != $ssl_session_id) {
			  tep_session_destroy();
			  tep_redirect(get_admin_link(PAGE_SSL_CHECK));
			}
		  }
...
// navigation history
	...
	if (isset($_GET['lang'])  ) {
		...
		$_SESSION['languages_id']		=	1; // EN by default
	}
	
	// language includes 
	$main_language	=	(isset($_SESSION['language_directory'])) ? $_SESSION['language_directory'] : _DEFAULT_ADMIN_LANGUAGE;

	if (is_file(_ADMIN_LANGUAGE_DIR.$main_language.'.php') ) {
		require(_ADMIN_LANGUAGE_DIR.$main_language.'.php');
	}
	
	if (is_file(_ADMIN_LANGUAGE_DIR.$main_language.'/'._CURRENT_ADMIN_MODULE._PAGE_EXTENDSION) ) {
		require(_ADMIN_LANGUAGE_DIR.$main_language.'/'._CURRENT_ADMIN_MODULE._PAGE_EXTENDSION);
	}
	...
	define('_IMAGES_LANG_DIR',_HTTP_ADMIN_SITE_ROOT.'/languages/'.$main_language.'/'); // Dir of language images 
